...
```{r }
knitr::kable(x = head(proportions$none), caption = 'The variants responsible for the 5 largest non-travel infections proportion',  booktabs = TRUE)
```
...
fractions peak at {first(x = maxima$Delta)} on {first(x = maxima$week)}; hereafter, it seems that the Delta fraction values are continuously, but slowly, declining.
...
The five variants that have the largest peak date fractions are^[Note: `Other` is not a variant, it represents a collection of variants, therefore excluded.]

\footnotesize

<!--- the variants and their peak dates --->
```{r}
knitr::kable(x = peaks, format = 'simple', digits = 3, caption = 'The variants responsible for the largest peak date fractions; w.r.t. data time span & non-travel data')
```
...
During the whole data time span, the five variants responsible for the five largest variant-infection--proportions are
